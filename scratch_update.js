...
const filepath = 'src/app/schedule/[userId]/page.tsx';
let content = fs.readFileSync(filepath, 'utf-8');

const new_generate = `function generateDaySlots(dateStr: string, durationMins: number = 60, bufferMins: number = 0, ownerTimezone: string = 'Asia/Kolkata') {
  const slots = [];
...
const start_idx = content.indexOf('function generateDaySlots');
const end_idx = content.indexOf('export default function UserSchedulePage()');
content = content.substring(0, start_idx) + new_generate + '\n\n' + content.substring(end_idx);

const state_str = '  const [selectedMeetingType, setSelectedMeetingType] = useState<any>(null);';
content = content.replace('const [bookName, setBookName] = useState("");', state_str + '\n  const [bookName, setBookName] = useState("");');

const effect_content = `      setProfile(targetProfile);
...
const allslots_old = 'const allSlots = useMemo(() => generateDaySlots(formattedDate), [formattedDate]);';
const allslots_new = `const allSlots = useMemo(() => {
...
const api_call_old = 'description: bookReason,';
const api_call_new = 'description: bookReason,\n          meetingType: selectedMeetingType?.name,';
content = content.replace(api_call_old, api_call_new);

const hero_ui = `      {/* User info */}
...
content = content.replace(old_hero, hero_ui);
